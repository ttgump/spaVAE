...
class SPAVAE(nn.Module):
    def __init__(self, input_dim, GP_dim, Normal_dim, encoder_layers, decoder_layers, noise, encoder_dropout, decoder_dropout, 
                    ...
                    KL_loss, init_beta, min_beta, max_beta, dtype, device):
...
#            gaussian_var = qnet_var[:, self.GP_dim:]
    ...
    def differential_expression(self, group1_idx, group2_idx, num_denoise_samples=10000, batch_size=512, pos=None, ncounts=None, 
        ...
        group1_denoised_counts = self.batching_denoise_counts(X=pos[group1_idx_sampling], Y=ncounts[group1_idx_sampling], batch_size=batch_size, n_samples=1)
        group2_denoised_counts = self.batching_denoise_counts(X=pos[group2_idx_sampling], Y=ncounts[group2_idx_sampling], batch_size=batch_size, n_samples=1)
    ...
    def train_model(self, pos, ncounts, raw_counts, size_factors, lr=0.001, weight_decay=0.001, batch_size=512, num_samples=1, 
        ...
        for epoch in range(maxiter):
            ...
            if train_size < 1:
                validate_elbo_val = 0
                validate_num = 0
                for _, (validate_x_batch, validate_y_batch, validate_y_raw_batch, validate_sf_batch) in enumerate(validate_dataloader):
