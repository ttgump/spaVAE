...
class SPAPEAKVAE(nn.Module):
    def __init__(self, input_dim, z_dim, encoder_layers, decoder_layers, noise, encoder_dropout, decoder_dropout, 
        ...
        if len(decoder_layers) > 0:
            ...
            self.dec_mean = nn.Sequential(nn.Linear(z_dim, input_dim), nn.Sigmoid())
    ...
    def batching_denoise_counts(self, X, Y, n_samples=1, batch_size=512, binary=False):
        """
    ...
    def train_model(self, pos, counts, lr=0.001, weight_decay=0.001, batch_size=512, num_samples=1, 
        ...
        for epoch in range(maxiter):
            ...
            print('Training epoch {}, ELBO:{:.8f}, BCE loss:{:.8f}, KLD loss:{:.8f}, noise regularization:{:8f}'.format(epoch+1, elbo_val, recon_loss_val, KL_term_val, noise_reg_val))
